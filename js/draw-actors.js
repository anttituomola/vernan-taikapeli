...
// Selkään päin: prinsessa yksisarvisen selässä, kasvoja ei näy.
function drawPrincessBack(c, x, y, s, walkPhase, moving, t) {
  c.save();
  c.translate(x, y);
  var sway = moving ? Math.sin(walkPhase) * 0.04 : Math.sin(t * 2) * 0.015;
  c.rotate(sway);
  c.beginPath();
  c.moveTo(0, -s * 16);
  c.quadraticCurveTo(-s * 16, -s * 2, -s * 12, s * 8);
  c.lineTo(s * 12, s * 8);
  c.quadraticCurveTo(s * 16, -s * 2, 0, -s * 16);
  c.closePath();
  artFillPath(c, DRESS, -s * 16, s * 8, s * 12);
  c.strokeStyle = '#ffd1e8';
  c.lineWidth = Math.max(1, s * 1.1);
  c.beginPath();
  c.moveTo(-s * 10, s * 4); c.quadraticCurveTo(0, s * 1, s * 10, s * 4);
  c.stroke();
  artBlob(c, 0, -s * 16, s * 6, s * 8, DRESS, {});
  artLimb(c, -s * 5, -s * 12, -s * 14, -s * 2, s * 3.1, SKIN, SKIN_LINE);
  artLimb(c, s * 5, -s * 12, s * 14, -s * 2, s * 3.1, SKIN, SKIN_LINE);
  artCircle(c, 0, -s * 30, s * 7.2, SKIN, { lineColor: SKIN_LINE, hi: 0.18 });
  artBlob(c, 0, -s * 32, s * 7.6, s * 6.4, HAIR, { hi: 0.22 });
  artBlob(c, -s * 6.5, -s * 20, s * 2.8, s * 8, HAIR, { rot: 0.25 });
  artBlob(c, s * 6.5, -s * 20, s * 2.8, s * 8, HAIR, { rot: -0.25 });
  artCircle(c, -s * 7.4, -s * 12, s * 1.7, '#ff8fbe', {});
  artCircle(c, s * 7.4, -s * 12, s * 1.7, '#ff8fbe', {});
  c.beginPath();
  c.moveTo(-s * 5.2, -s * 38);
  c.lineTo(-s * 5.2, -s * 44);
  c.lineTo(-s * 2.4, -s * 40.5);
  c.lineTo(0, -s * 46);
  c.lineTo(s * 2.4, -s * 40.5);
  c.lineTo(s * 5.2, -s * 44);
  c.lineTo(s * 5.2, -s * 38);
  c.closePath();
  artFillPath(c, '#ffd24f', -s * 46, -s * 38, s * 4.5, { lineColor: '#d98a00' });
  c.fillStyle = '#ff5f7e';
  c.beginPath(); c.arc(0, -s * 39.6, s * 1, 0, Math.PI * 2); c.fill();
  c.restore();
}

function drawUnicornBack(c, x, y, s, walkPhase, moving, t) {
  var swing = moving ? Math.sin(walkPhase) * s * 5 : 0;
  artShadow(c, x, y + s * 10, s * 32, s * 9, 0.16);
  c.save();
  c.translate(x, y);
  c.translate(0, moving ? -Math.abs(Math.sin(walkPhase)) * s * 2 : 0);
  artLimb(c, -s * 12, s * 2, -s * 14 + swing * 0.3, s * 20, s * 5.2, UNI_SHADE, UNI_LINE);
  artLimb(c, s * 12, s * 2, s * 14 - swing * 0.3, s * 20, s * 5.2, UNI_SHADE, UNI_LINE);
  artBlob(c, 0, -s * 6, s * 24, s * 16, UNI_WHITE, { shadeTo: UNI_SHADE, lineColor: UNI_LINE, hi: 0.32 });
  artLimb(c, -s * 9, s * 4, -s * 11 - swing, s * 22, s * 5.8, UNI_WHITE, UNI_LINE);
  artLimb(c, s * 9, s * 4, s * 11 + swing, s * 22, s * 5.8, UNI_WHITE, UNI_LINE);
  artBlob(c, 0, s * 12, s * 6, s * 9, '#c9a0ff', { shadeTo: '#8a6ab8', lineColor: '#6a4a98', hi: 0.2 });
  artBlob(c, 0, -s * 26, s * 11, s * 10, UNI_WHITE, { shadeTo: UNI_SHADE, lineColor: UNI_LINE, hi: 0.28 });
  artBlob(c, -s * 8, -s * 36, s * 4.2, s * 7, UNI_WHITE, { rot: -0.35, lineColor: UNI_LINE });
  artBlob(c, s * 8, -s * 36, s * 4.2, s * 7, UNI_WHITE, { rot: 0.35, lineColor: UNI_LINE });
  artLimb(c, 0, -s * 34, 0, -s * 50, s * 3.2, '#ffe27a', '#d98a00');
  drawPrincessBack(c, 0, -s * 20, s * 0.95, walkPhase, moving, t);
  c.restore();
}
